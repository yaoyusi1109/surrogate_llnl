...
How does linear interpolation work in higher dimension?  We could use the Delaunay triangulation, then use linear interpolation of each triangle containing a predictive location.  This would get very messy.  Maybe we could do it for small dimensional problems only?

```{r}
library(geometry)
tri <- delaunayn(x)
onetri <- x[tri[1, ], ]
hull <- convhulln(onetri)
inhulln(hull, xstar)

tsearch(x[, 1], x[, 2], delaunayn(x), xstar[1], xstar[2])


x <- randomLHS(10, 4)
xstar <- matrix(c(0.5, 0.5, 0.5, 0.5), nrow = 1)
tsearchn(x, delaunayn(x), xstar)

# monotonic can work for only 1, 2, or 3 dimensions
```
This will get very messy in higher dimensions.  For now, we will just offer 1 and 2 dimensions?

```{r}
get_interp <- function(x, w, xnew, index = NULL, tri = NULL, qhull = NULL) {
  # x: matrix of original training locations
  # w: matrix of warped training locations (each column will be handled independently)
  # xnew: matrix of testing locations that need to be warped through interpolation
  if (!is.matrix(x)) x <- as.matrix(x)
  if (!is.matrix(w)) w <- as.matrix(w)
  if (!is.matrix(xnew)) xnew <- as.matrix(xnew)
  d <- ncol(x)
  if (d == 1) {
    if (is.null(index)) index <- deepgp:::fo_approx_init(x, xnew)
    wnew <- deepgp:::fo_approx(x, w, xnew, index)
  } else if (d == 2) {
    i <- interp::interp(x[, 1], x[, 2], w, xo = xnew[, 1], yo = xnew[, 2],
                        output = "points", method = "akima", extrap = TRUE)
    # but how to extrapolate? I think I'll need to create my own function for that
    # or could we just use kriging?  But it wouldn't be monotonic smh
  } else {
    stop("linear interpolation only offered for d = 1:3")
  }
}
